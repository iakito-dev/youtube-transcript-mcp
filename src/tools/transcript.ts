...
async function getTranscriptWithAutoDetection(videoId: string, env: any): Promise<string> {
  // English first; if YouTube has it in another language it tells us which
  // ones in the error, so there is no need to guess through a fixed list.
  const attempted: string[] = [];
  let candidates = ['en'];
  let lastError: any;

  while (candidates.length > 0) {
    const lang = candidates.shift() as string;
    if (attempted.includes(lang)) continue;
    attempted.push(lang);
    ...
    try {
      ...
      // Anything other than "wrong language" fails identically for every
      // language, so stop instead of burning subrequests on the same answer.
      if (!(error instanceof LanguageUnavailableError)) {
        break;
      }

      // Queue the languages YouTube says it actually has.
      candidates = error.availableLanguages.filter(
        (candidate: string) => !attempted.includes(candidate)
      );
    }
  }

  throw new Error(handleYouTubeErrors(lastError));
}
...
async function getTranscriptWithFallback(videoId: string, env: any, requestedLanguage: string): Promise<string> {
  try {
    ...
    if (error instanceof LanguageUnavailableError && requestedLanguage !== 'en') {
