 ...
 */
/**
 * Raised when the video has transcripts, but not in the language that was
 * asked for. Carries the languages YouTube does offer, so the caller can
 * retry one that exists instead of guessing.
 */
export class LanguageUnavailableError extends Error {
  availableLanguages: string[];

  constructor(message: string, availableLanguages: string[] = []) {
    super(message);
...
/**
 * Pulls the language list out of the library's
 * "... Available languages: en,de,fr" error message.
 */
function parseAvailableLanguages(message: string): string[] {
  const match = /Available languages\s*:\s*(.+)$/im.exec(message ?? '');
  if (!match) return [];
...
export async function getTranscript(videoId: string, language: string = 'en'): Promise<string> {
  ...
  while (attempts < MAX_RETRIES) {
    try {
      ...
      // youtube-transcript raises distinct error types. Classify on those
      // rather than on message text: only a missing track for the requested
      // language is worth trying another language, and none of these failures
      // get better by repeating the identical request.
      switch (error.name) {
        case 'YoutubeTranscriptNotAvailableLanguageError':
          throw new LanguageUnavailableError(
            `No transcript in ${language} for ${videoId}.`,
            parseAvailableLanguages(error.message)
          );

        case 'YoutubeTranscriptVideoUnavailableError':
          throw new InvalidVideoError(`Video ${videoId} is unavailable, private or deleted.`);

        case 'YoutubeTranscriptDisabledError':
          throw new VideoUnavailableError(`Transcripts are disabled for ${videoId}.`);

        case 'YoutubeTranscriptNotAvailableError':
          throw new VideoUnavailableError(`No transcripts are available for ${videoId}.`);

        case 'YoutubeTranscriptTooManyRequestError':
          if (attempts >= MAX_RETRIES) {
            throw new RateLimitError(`Rate limited by YouTube after ${attempts} attempts.`);
          }
